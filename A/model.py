...
def run_model_A(train_images, train_labels, val_images, val_labels, test_images, test_labels):
    print("Model A: Feature Engineering Comparative Testing")
    ...
    # test 1: Raw Pixels
    acc_raw = run_rf_experiment(X_train_raw, y_train, 
                                X_val_raw, y_val, 
                                X_test_raw, y_test, 
                                "Baseline (Raw Pixels)")
    ...
    print(f"Data shape variation: {X_train_raw.shape} -> {X_train_pca.shape}")
    
    acc_pca = run_rf_experiment(X_train_pca, y_train, 
                                X_val_pca, y_val, 
                                X_test_pca, y_test, 
    ...
    if acc_pca > acc_raw:
        print("Conclusion: Principal Component Analysis (PCA) has enhanced the model's accuracy.")
    else:
        print("Conclusion: Principal Component Analysis (PCA) slightly reduced accuracy but significantly reduced the dimensionality of features.")
        
    return {"raw_acc": acc_raw, "pca_acc": acc_pca}